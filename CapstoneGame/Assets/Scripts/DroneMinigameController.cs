    ...
    void Update()
    {
        if (insideRange && Input.GetKeyDown(KeyCode.E))
        {
            ...
            if (CompletedCheckpoints == false)
            {
                ...
                subText.GetComponent<Text>().text = "Alright, so Rikki says he needs a nice wide angle shot of the land. Show me what you’ve got! Just steer along those flags that I've set up and I'll take the photos of what we need.";
            ...
            else
            {
                ...
                Cursor.lockState = CursorLockMode.Confined;
                subBox.SetActive(true);
                subText.GetComponent<Text>().text = "That photo should be the ticket. Why don't you go ahead and take it back to Rikki";
                characterName.GetComponent<Text>().text = "Fred";
